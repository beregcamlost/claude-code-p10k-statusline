...
cwd=${cwd//$'\r'/}   # strip a stray CR (CRLF-edited config)
# Normalize Windows paths so $HOME / home / folder detection works under Git-bash:
...
case $glyphs in
  emoji)
    ...
    # Keep powerline chevrons: emoji terminals (Warp, modern ones) render them too.
    # Truly bare terminals (no powerline) should use ascii mode instead.
    PSEP_THIN=$'\xee\x82\xb1'; PSEP_END=$'\xee\x82\xb0'
...
# Intra-group spacing (OS->path, path->git): one space for double-width emoji/ascii
# glyphs (their own cell padding already adds visual gap), two for narrow nerd glyphs.
